...
static const char *my_device = "my_cedv";
...
static int my_open(struct inode *pInode, struct file *pFile){
    pr_info("%s: my_open called, Mojor: %d, inor: %d\n", my_device, imajor(pInode), iminor(pInode));

    pr_info("%s: file->f_ops: %lld\n", my_device, pFile->f_pos);
...
static int __init my_init(void)
{
    ...
    // initialise fileoperation  and declare onwership to prevent losing chrdev 
    cdev_init(&my_cdev, &fops);
    my_cdev.owner = THIS_MODULE;

    status = cdev_add(&my_cdev ,dev_nr, MINORMASK+1);
    if(status){
        pr_err("%s: error ading cdev\n", my_device);
        goto free_device_nr;
    }

    my_class = class_create("my_class");
    if(!my_class){
        pr_err("%s: Could not create my_class\n",my_device);
        status = ENOMEM;
        goto delete_cdev;
    }

    if(!device_create(my_class,NULL,dev_nr, NULL,"my_cdev%d", 0)){
        pr_err("%s: Could not create device my_cdv0\n",my_device);
        status = ENOMEM;
        goto delete_class;
    }

    pr_info("%s: Character device registered, Major number : %d MInor number: %d\n", my_device, MAJOR(dev_nr), MINOR(dev_nr));
    pr_info("%s: Created device number  /sys/class/my_class\n", my_device);
